...
from coilutils.general import camelcase_to_snakecase
...
def read_and_print_summary(path, weathers):
    """
        We plot the summary of the testing for the set selected weathers.
    ...
    with open(os.path.join(path, 'metrics.json')) as json_file:
        metrics_summary = json_file.read()
...
if __name__ == '__main__':
    ...
    argparser.add_argument(
        ...
        dest='path',
        required=True,
        help= "path to directory containing metrics.json file"
    )
    argparser.add_argument(
        '-e', '--exp_set_name',
        dest='exp_set_name',
        required=True,
        help= "driving suite used in camel case (see drive/suites)"
    )
    
    args = argparser.parse_args()

    experiment_suite_module = __import__('drive.suites.' + camelcase_to_snakecase(args.exp_set_name)
                                            + '_suite',
                                            fromlist=[args.exp_set_name])
    experiment_suite_module = getattr(experiment_suite_module, args.exp_set_name)

    experiment_set = experiment_suite_module()
    
    print("\n\n----- Printing results for training weathers (Seen in Training) -----\n\n")
    # weathers = [int(i) for i in experiment_set.train_weathers]
    read_and_print_summary(args.path, experiment_set.train_weathers)
    
    print("\n\n----- Printing results for test weathers (Unseen in Training) -----\n\n")
    # weathers = [int(i) for i in experiment_set.test_weathers]
    read_and_print_summary(args.path, experiment_set.test_weathers)